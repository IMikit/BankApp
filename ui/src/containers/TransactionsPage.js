...
class TransactionsPage extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <FlatButton onClick={() => this.goToAccounts()} label="Revenir aux comptes" primary={true} />

                <h2>{account.name} Transactions</h2>

                <Paper zDepth={1}>
                    <Subheader>Solde du compte {formatMoney(account.balance)}</Subheader>
                    {
                        transactions.length === 0 &&
                        <Subheader>Pas de transaction disponible</Subheader>
                    }
                    {
                        transactions.length > 0 &&
